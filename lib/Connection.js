...
Connection.prototype.connect = function() {
  ...
  parser.on('continue', function(info) {
    if (!self._curReq) {
      self.debug && self.debug('failed to continue, no curReq object');
      return;
    }
    ...
    if (type === 'IDLE') {
      ...
      self._curReq.oauthError = new Buffer(info.text, 'base64').toString(
        'utf8'
      );
  ...
  parser.on('other', function(line) {
    ...
    if ((m = RE_IDLENOOPRES.exec(line))) {
      ...
      clearTimeout(self._tmrKeepalive);
      this.debug && this.debug('other event destroys curReq');
...
Connection.prototype.destroy = function() {
  this._queue = [];
  this.debug && this.debug('destroying IMAP connection');
  this._curReq = undefined;
...
Connection.prototype.end = function() {
  ...
  this._enqueue('LOGOUT', function() {
    self._queue = [];
    this.debug && this.debug('logged out IMAP connection');
    self._curReq = undefined;
    self._sock.end();
  });
};
...
Connection.prototype.renameBox = function(oldname, newname, cb) {
  ...
  this._enqueue('RENAME "' + encoldname + '" "' + encnewname + '"', function(
    err
  ) {
...
Connection.prototype.setQuota = function(quotaRoot, limits, cb) {
  ...
  this._enqueue('SETQUOTA "' + quotaRoot + '" (' + triplets + ')', function(
    err,
    quotalist
  ) {
    if (err) return cb(err);
...
Connection.prototype._resUntagged = function(info) {
  var type = info.type,
    ...
    key;
  if (!this._curReq) {
    this.debug && this.debug('failed to resUntagged, no curReq object');
    //   // return;
  }
...
Connection.prototype._resTagged = function(info) {
  ...
  this.debug && this.debug('resTagged undefined curReq');
  this._curReq = undefined;
